...
export default async function ChatLayout({
  ...
  // 3. Fetch Projects with typed results
  ...
  // Cast the Supabase response to our clean interface
  const projectMemberships = data as unknown as ProjectMembership[];

  return (
    <div className="flex h-[100dvh] flex-col lg:flex-row overflow-hidden bg-white">
      
      {/* 1. Sidebar Wrapper */}
      <SidebarWrapper orgSlug={org_slug}>
        <div className="p-4 border-b border-slate-200 bg-white/50 shrink-0">
          <Link 
            href="/dashboard" 
            className="flex items-center gap-2 text-xs font-semibold text-slate-500 hover:text-indigo-600 transition-colors mb-4"
          >
